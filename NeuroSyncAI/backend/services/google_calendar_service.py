...
db = client["neurosyncai"]
tokens_collection = db["google_tokens"]

def get_stored_tokens():
    """Retrieve stored tokens from MongoDB."""
    token_data = tokens_collection.find_one({"user_id": "default"})
...
def refresh_google_calendar_token():
    """Automatically refresh expired OAuth2 tokens."""
    stored_tokens = get_stored_tokens()
    
    if not stored_tokens or "refresh_token" not in stored_tokens:
        raise HTTPException(status_code=401, detail="No refresh token available. Please reauthenticate.")

    credentials = Credentials(
        token=stored_tokens["access_token"],
        refresh_token=stored_tokens["refresh_token"],
        token_uri="https://oauth2.googleapis.com/token",
        client_id=stored_tokens["client_id"],
        client_secret=stored_tokens["client_secret"]
    )
